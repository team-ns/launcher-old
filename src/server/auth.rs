...
pub(crate) async fn join(
    ...
    match entry {
        Ok(e) => {
            if e.access_token.is_some() && e.access_token.unwrap().eq(&request.access_token) {
                provide
                    ...
                    .await;
                Ok(warp::reply::with_status(warp::reply::json(&Value::default()), StatusCode::OK))
            } else {
        ...
        Err(error) => Ok(warp::reply::with_status(warp::reply::json(&serde_json::json!({
             "error": error.msg
             ,
...
pub(crate) async fn has_join(
    ...
    match entry {
        Err(_e) => Ok(warp::reply::with_status(warp::reply::json(&Value::default()), StatusCode::BAD_REQUEST)),
        Ok(e) => {
            if e.server_id.is_some() && e.server_id.clone().unwrap().eq(&request.server_id) {
                ...
                Ok(warp::reply::with_status(warp::reply::json(&Value::default()), StatusCode::BAD_REQUEST))
            }
        }
    }
}
